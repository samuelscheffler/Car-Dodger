...
public class GamePanel extends Canvas implements Runnable
{
	...
	private static final double MOVEMENT_SPEED=(100*(REFRESH_SPEED/1000.0));
	private static final double SCROLL_SPEED=(150*(REFRESH_SPEED/1000.0));
	private static final int NUM_OF_ENEMY_CARS=5;
	private static final double DISTANCE_BETWEEN_ENEMY_CARS=360;
	...
	public void run()
	{
		...
		{
			now=System.nanoTime();
			double delta=(now-lastUpdate)/(double)TARGET_TIME;
			time+=now-lastUpdate;
			lastUpdate=now;
	...
	public void update(double delta)
	{
		...
		scroller.setScrollSpeed(SCROLL_SPEED*delta);		
		spawner.setMovementSpeeds((SCROLL_SPEED*2)*delta);
		
		listener.update();	
		scroller.scroll();
		spawner.moveCars();
		spawner.checkCollison(playerCar);
	}
	...
	public void render()
	{
		...
		if(spawner.checkCollison(playerCar))
		{
